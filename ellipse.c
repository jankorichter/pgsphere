...
*/
  ...
  */
  static float8 my_acos ( float8 a ){
      if ( a >  1.0 ) a =  1.0;
      if ( a < -1.0 ) a = -1.0;
      return acos( a ) ;
  }
  ...
  */
  static SELLIPSE * sellipse_in( float8 r1, float8 r2, const SPoint * c, float8 inc )
  {
  ...
  */
  static float8  sellipse_point_dist ( const SELLIPSE * se, const SPoint * sp )
  {
    static SEuler e ;
    static SPoint p ;
  ...
  */
  ...
  /*!
    ...
    \return relationship as a \link PGS_ELLIPSE_ELLIPSE_REL int8 value \endlink (\ref PGS_ELLIPSE_ELLIPSE_REL )
  */
  static int8 sellipse_ellipse_pos ( const SELLIPSE * se1 , const SELLIPSE * se2 )
  {
     static int8    r;
     ...
     if ( FPeq ( se2->rad[0], se2->rad[1] ) ) {
       ...
       switch ( r ){
         case PGS_ELLIPSE_CIRCLE_AVOID : return PGS_ELLIPSE_AVOID ;
         case PGS_CIRCLE_CONT_ELLIPSE  : return PGS_ELLIPSE_OVER  ;
         case PGS_ELLIPSE_CONT_CIRCLE  : return PGS_ELLIPSE_CONT  ;
         case PGS_ELLIPSE_CIRCLE_OVER  : return PGS_ELLIPSE_OVER  ;
         default                       : return PGS_ELLIPSE_AVOID ;
       }
     ...
     if ( FPeq ( se1->rad[0], se1->rad[1] ) ) {
       ...
       switch ( r ){
         case PGS_ELLIPSE_CIRCLE_AVOID : return PGS_ELLIPSE_AVOID ;
         case PGS_CIRCLE_CONT_ELLIPSE  : return PGS_ELLIPSE_CONT  ;
         case PGS_ELLIPSE_CONT_CIRCLE  : return PGS_ELLIPSE_OVER  ;
         case PGS_ELLIPSE_CIRCLE_OVER  : return PGS_ELLIPSE_OVER  ;
         default                       : return PGS_ELLIPSE_AVOID ;
       }
     ...
     if ( FPzero ( se2->rad[1] ) ) {
       ...
       switch ( r ){
         case PGS_ELLIPSE_LINE_AVOID   : return PGS_ELLIPSE_AVOID ;
         case PGS_ELLIPSE_CONT_LINE    : return PGS_ELLIPSE_CONT  ;
         case PGS_ELLIPSE_LINE_OVER    : return PGS_ELLIPSE_OVER  ;
         default                       : return PGS_ELLIPSE_AVOID ;
       }
     ...
     if ( FPzero ( se1->rad[1] ) ) {
       ...
       switch ( r ){
         case PGS_ELLIPSE_LINE_AVOID   : return PGS_ELLIPSE_AVOID ;
         case PGS_ELLIPSE_CONT_LINE    : return PGS_ELLIPSE_OVER  ;
         case PGS_ELLIPSE_LINE_OVER    : return PGS_ELLIPSE_OVER  ;
         default                       : return PGS_ELLIPSE_AVOID ;
       }
     ...
     do {
       ...
       if ( FPle( ( dist + se1->rad[0] ) , se2->rad[1] ) ){
         return PGS_ELLIPSE_OVER  ;
       } else
       if ( FPle( ( dist + se2->rad[0] ) , se1->rad[1] ) ){
         return PGS_ELLIPSE_CONT  ;
       ...
       // transform se1 to north pol
         ...
         do {
           ...
           if ( diff[1] <= diff[0] && diff[0] <= diff[2] ){
              memcpy( (void*) &sp[2], (void*) &sp[0], sizeof( SPoint ) );
              memcpy( (void*) &sp[0], (void*) &sp[1], sizeof( SPoint ) );
           } else
           if ( diff[1] <= diff[2] && diff[2] <= diff[0] ){
              memcpy( (void*) &sp[0], (void*) &sp[1], sizeof( SPoint ) );
           } else
           if ( diff[2] <= diff[0] && diff[0] <= diff[1] ){
              if ( pgs_abs(sp[0].lng-elng) < pgs_abs(sp[2].lng-elng) ) {
                  memcpy( (void*) &sp[2], (void*) &sp[1], sizeof( SPoint ) );
              } else {
                  memcpy( (void*) &sp[0], (void*) &sp[1], sizeof( SPoint ) );
  ...
  bool sellipse_eq ( const SELLIPSE * e1 , const SELLIPSE * e2 )
  {
    ...
    if ( FPeq ( e1->rad[0], e1->rad[1] ) ){ // circle
      static SCIRCLE c[2];
  ...
  int8 sellipse_circle_pos ( const SELLIPSE * se , const SCIRCLE * sc )
  {
    ...
    if ( FPzero(se->rad[1]) ){
      ...
      if ( FPzero(dist) ){
        ...
        if ( FPle( (dist + a ), sc->radius ) ) {
          return PGS_CIRCLE_CONT_ELLIPSE;
        } else 
        if ( FPgt( (a + sc->radius), dist ) ) {
          return PGS_ELLIPSE_CIRCLE_OVER;
        } else {
          return PGS_ELLIPSE_CIRCLE_AVOID;
        }
